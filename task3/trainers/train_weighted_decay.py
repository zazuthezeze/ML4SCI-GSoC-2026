...
results_dir = r'C:\Users\Programmer2\Desktop\Specific_Task1_Anomaly detection\results\weightdecay_256_30epochs'
...
torch.save(model.state_dict(), os.path.join(results_dir, 'contrastive_model_256_30epochs.pth'))
print("Model saved!")
...
torch.save(classifier.state_dict(), os.path.join(results_dir, 'classifier_256_30epochs.pth'))
...
plt.plot(fpr, tpr, linewidth=2, label=f'Contrastive + Weight Decay 256dim (AUC = {auc:.4f})')
plt.plot([0, 1], [0, 1], 'k--', label='Random classifier')
plt.xlabel('False Positive Rate', fontsize=12)
plt.ylabel('True Positive Rate', fontsize=12)
plt.title('ROC Curve — Contrastive Learning + Weight Decay 256dim', fontsize=14)
...
plt.title('Contrastive Training Loss Curve — Weight Decay 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Classifier Training Loss Curve — Weight Decay 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title('Classifier Training Accuracy Curve — Weight Decay 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
with open(os.path.join(results_dir, 'auc_score.txt'), 'w') as f:
    f.write(f'Contrastive Learning + Weight Decay 256dim 30 Epochs — Test ROC-AUC: {auc:.4f}\n')
