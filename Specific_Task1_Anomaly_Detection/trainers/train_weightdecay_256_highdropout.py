...
from model_highdropout import ContrastiveModel, ClassifierHighDropout
from data import get_contrastive_loaders

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

results_dir = r'C:\Users\Programmer2\Desktop\Specific_Task1_Anomaly detection\results\weightdecay_256_highdropout_30epochs'
...
torch.save(model.state_dict(), os.path.join(results_dir, 'contrastive_model_256_highdropout.pth'))
print("Model saved!")

for param in model.encoder.parameters():
    param.requires_grad = False

classifier = ClassifierHighDropout(latent_dim=256).to(device)
...
torch.save(classifier.state_dict(), os.path.join(results_dir, 'classifier_256_highdropout.pth'))
...
plt.plot(fpr, tpr, linewidth=2, label=f'Contrastive + WD + HighDropout 256dim (AUC = {auc:.4f})')
plt.plot([0, 1], [0, 1], 'k--', label='Random classifier')
plt.xlabel('False Positive Rate', fontsize=12)
plt.ylabel('True Positive Rate', fontsize=12)
plt.title('ROC Curve — Contrastive + Weight Decay + High Dropout 256dim', fontsize=14)
...
plt.title('Contrastive Training Loss Curve — WD + High Dropout 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Classifier Training Loss Curve — WD + High Dropout 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.title('Classifier Training Accuracy Curve — WD + High Dropout 256dim')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
...
with open(os.path.join(results_dir, 'auc_score.txt'), 'w') as f:
    f.write(f'Contrastive + Weight Decay + High Dropout 256dim 30 Epochs — Test ROC-AUC: {auc:.4f}\n')
